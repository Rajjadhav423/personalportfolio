
export const portfolioData = {
  personal: {
    name: "Rajesh Jadhav",
    role: "Full Stack Developer",
    roles: [
      "Software Developer",
      "Full Stack Engineer",
      "Salesforce Developer",
    ],
    description: "Aspiring Software Developer with hands-on experience in full-stack web development, Salesforce CRM platform development, and machine learning research.",
    aboutDescription: {
      p1: "Hi! I'm Rajesh Jadhav, an aspiring Full Stack Developer and Machine Learning enthusiast with a passion for creating cutting-edge software solutions.",
      p2: "Currently a Software Engineer at Koshine Tech Labs, I specialize in Java, Python, and MERN stack development. I am a 3x Certified Salesforce Developer committed to building scalable, efficient, and innovative web applications.",
      p3: "My mission is to push the boundaries of technology—merging conversational AI with seamless user experiences and streamlining complex workflows through code. I am a fast learner, problem-solver, and ready to make an impact."
    },
    location: "Pune, Maharashtra, India",
    email: "jadhavrz423@gmail.com",
    phone: "+91-9322850587",
    linkedin: "https://www.linkedin.com/in/rajeshjadhav057",
    github: "https://github.com/Rajjadhav423",
    medium: "https://medium.com/@jadhavrz423",
  },
  
  stats: [
    { value: "4+", label: "Projects Built" },
    { value: "1+", label: "Year Experience" },
    { value: "3", label: "Salesforce Certs" },
    { value: "3x", label: "Certified Dev" },
  ],

  experiences: [
    {
      company: "Koshine Tech Labs",
      role: "Software Engineer",
      duration: "Nov 2025 – Present",
      location: "Pune, India",
      type: "Full-time",
      description: "Architecting scalable enterprise solutions at the intersection of MERN stack and Salesforce ecosystem, driving product innovation for B2B SaaS platforms.",
      highlights: [
        {
          text: "Engineered 'Replydock', a multi-tenant customer communication platform integrating Slack & WhatsApp APIs, reducing response time by 60% across 500+ active users",
          metric: "60% faster response",
          icon: "rocket",
        },
        {
          text: "Architected event-driven microservices using Node.js, Apache Kafka, and PostgreSQL, achieving 99.9% uptime and handling 10K+ daily message events",
          metric: "10K+ events/day",
          icon: "server",
        },
        {
          text: "Designed and deployed custom Lightning Web Components (LWC) and Apex triggers for 3 enterprise clients, automating 40+ manual workflows",
          metric: "40+ workflows automated",
          icon: "lightning",
        },
        {
          text: "Implemented OAuth 2.0 authentication flows and end-to-end encryption, ensuring SOC 2 compliance-ready security architecture",
          metric: "Enterprise-grade security",
          icon: "shield",
        },
      ],
      technologies: ["React", "Node.js", "Salesforce", "PostgreSQL", "Kafka", "LWC", "Apex"],
      color: "cyan",
    },
    {
      company: "Koshine Tech Labs",
      role: "Software Developer Intern",
      duration: "Feb 2025 – Oct 2025",
      location: "Pune, India",
      type: "Internship",
      description: "Accelerated from learning to shipping production-ready code within 4 weeks, contributing to core product features and platform reliability.",
      highlights: [
        {
          text: "Spearheaded development of 'CRUDSpace', a multi-org Salesforce management tool handling metadata operations across 15+ connected orgs",
          metric: "15+ orgs managed",
          icon: "grid",
        },
        {
          text: "Optimized PostgreSQL queries and implemented connection pooling, reducing database response time by 40% for high-traffic endpoints",
          metric: "40% faster queries",
          icon: "database",
        },
        {
          text: "Built reusable React component library with TypeScript, adopted across 3 internal projects and reducing frontend dev time by 25%",
          metric: "25% dev time saved",
          icon: "code",
        },
        {
          text: "Established CI/CD pipelines using GitHub Actions, achieving 95% test coverage and zero-downtime deployments",
          metric: "95% test coverage",
          icon: "check",
        },
      ],
      technologies: ["React", "TypeScript", "PostgreSQL", "jsforce", "GitHub Actions", "Prisma"],
      color: "blue",
    },
    {
      company: "BITS Pilani Goa Campus",
      role: "Research Intern",
      duration: "Jan 2025 – May 2025",
      location: "Goa, India",
      type: "Research",
      description: "Pioneered AI-driven research at India's premier technical institute, bridging machine learning with sustainable chemistry applications.",
      highlights: [
        {
          text: "Developed 'Predictive Lignin Dissolution Model' using fine-tuned LLMs, achieving 92% prediction accuracy for sustainable biomass processing",
          metric: "92% accuracy",
          icon: "brain",
        },
        {
          text: "Designed conversational AI chatbot for thermodynamic property prediction, processing 1000+ scientific queries with real-time analysis",
          metric: "1000+ queries processed",
          icon: "message",
        },
        {
          text: "Published research findings contributing to 2 ongoing academic papers on AI applications in green chemistry",
          metric: "2 academic papers",
          icon: "book",
        },
        {
          text: "Built data pipelines processing 50K+ experimental data points using Python, TensorFlow, and scikit-learn for interdisciplinary research",
          metric: "50K+ data points",
          icon: "chart",
        },
      ],
      technologies: ["Python", "TensorFlow", "scikit-learn", "LLMs", "Pandas", "NumPy"],
      color: "purple",
    },
  ],

  education: {
    degree: "Bachelor of Technology in Information Technology",
    university: "SGGS Institute of Engineering and Technology, Nanded",
    year: "2021 – 2025",
    cgpa: "CGPA: 7.94/10",
    certifications: [
      {
        title: "Salesforce Certified Administrator",
        issuer: "Salesforce",
        year: "2025",
        color: "cyan",
      },
      {
        title: "Salesforce AI Associate",
        issuer: "Salesforce",
        year: "2025",
        color: "purple",
      },
      {
        title: "Salesforce Agentforce",
        issuer: "Salesforce",
        year: "2025",
        color: "pink",
      },
      {
        title: "Full Stack Web Development (MERN)",
        issuer: "Certification Course",
        year: "2024",
        color: "blue",
      },
      {
        title: "Java Application Development",
        issuer: "Innovative Products Workshop",
        year: "2023",
        color: "orange",
      },
      {
        title: "Introduction to Programming Using Python",
        issuer: "Certification",
        year: "Completed",
        color: "green",
      },
    ],
    coursework: [
      "Data Structures & Algorithms",
      "Database Management",
      "Software Engineering",
      "Computer Networks",
      "Machine Learning",
      "Object Oriented Programming",
    ]
  },

  skills: [
    {
      title: "Programming Languages",
      skills: ["JavaScript", "TypeScript", "Python", "Java", "C++", "SQL", "HTML5", "CSS3"],
      color: "cyan",
    },
    {
      title: "Frontend Technologies",
      skills: ["React.js", "Next.js", "LWC", "Tailwind CSS", "Bootstrap", "Responsive Design"],
      color: "purple",
    },
    {
      title: "Backend Technologies",
      skills: ["Node.js", "Express.js", "Apache Kafka", "RESTful APIs", "Microservices"],
      color: "pink",
    },
    {
      title: "Salesforce Technologies",
      skills: ["Apex", "Salesforce Admin", "jsforce", "SOQL/SOSL", "Lightning Platform"],
      color: "blue",
    },
    {
      title: "Databases & Storage",
      skills: ["PostgreSQL", "MongoDB", "MySQL", "Supabase", "Prisma ORM"],
      color: "green",
    },
    {
      title: "Cloud & DevOps",
      skills: ["AWS", "Git", "GitHub", "Slack Integration", "Docker", "CI/CD", "OAuth", "JWT"],
      color: "orange",
    },
    {
      title: "Machine Learning",
      skills: ["Supervised Learning", "Unsupervised Learning", "Reinforcement Learning", "TensorFlow", "scikit-learn"],
      color: "red",
    },
  ],

  projects: [
    {
      title: "Cloud Force CRM Manager (CFCRM)",
      description: "AI-powered Salesforce CRM platform with Sales Cloud & Service Cloud support, natural language query assistant, schema visualization, multi-org management, and secure OAuth authentication.",
      tech: ["Next.js", "jsforce", "Salesforce API", "AI Integration", "Tailwind CSS"],
      date: "May 2025",
      features: ["Natural Language Query", "Schema Visualization", "Multi-Org Management", "Real-time Analytics"],
      gradient: "from-amber-500 to-orange-500",
    },
    {
      title: "Uber Clone - Ride Sharing App",
      description: "Full-stack ride-sharing application with real-time booking, vehicle selection, fare estimation, live GPS tracking, and responsive React.js frontend.",
      tech: ["React.js", "Node.js", "Socket.IO", "Google Maps API", "JWT"],
      date: "November 2024",
      features: ["Real-time Tracking", "Live Booking", "Fare Estimation", "Driver Matching"],
      gradient: "from-purple-500 to-pink-500",
    },
    {
      title: "StudyNotion - EdTech Platform",
      description: "Educational platform for course creation, enrollment, content management, and progress tracking. Integrated secure Razorpay payments supporting 1000+ users.",
      tech: ["MERN Stack", "Razorpay", "MongoDB", "Express.js", "React.js"],
      date: "November 2023",
      features: ["Course Management", "Payment Integration", "User Authentication", "Rating System"],
      gradient: "from-orange-500 to-red-500",
    },
  ]
};
