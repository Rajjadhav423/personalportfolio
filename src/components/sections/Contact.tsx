...
export function Contact() {
  ...
  return (
    <section id="contact" className="py-16 md:py-24 section-gradient relative overflow-hidden bg-white dark:bg-zinc-950 transition-colors duration-300">
      {/* Background */}
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid lg:grid-cols-2 gap-8 md:gap-12">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-1 gap-3 sm:gap-4">
              {contactInfo.map((info, idx) => (
                <SpotlightCard
                  ...
                  spotlightColor={
                    info.color === 'cyan' ? 'rgba(6, 182, 212, 0.15)' :
                    info.color === 'purple' ? 'rgba(139, 92, 246, 0.15)' :
                    info.color === 'blue' ? 'rgba(59, 130, 246, 0.15)' :
                ...
                >
                  <a
                    ...
                    className="flex items-center gap-3 sm:gap-4 p-3 sm:p-4 group"
                  >
                    <div className={`p-2 sm:p-3 rounded-lg ${
                      ...
                      'bg-zinc-500/10 text-zinc-600 dark:text-zinc-400 group-hover:bg-zinc-500/20'
                    } transition-colors`}>
                      <info.icon size={20} className="sm:w-6 sm:h-6" />
                    </div>
                    <div className="min-w-0">
                      <p className="text-zinc-500 dark:text-zinc-500 text-xs sm:text-sm">{info.label}</p>
          ...
          >
            <SpotlightCard className="p-4 sm:p-6 md:p-8" spotlightColor="rgba(139, 92, 246, 0.1)">
              <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
                <div>
                  <label htmlFor="name" className="block text-sm font-medium text-zinc-600 dark:text-zinc-400 mb-2">
                    Name
                  </label>
                  <input
                    type="text"
                    ...
                    className="input-field w-full px-3 sm:px-4 py-2.5 sm:py-3 rounded-lg focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/50 transition-all outline-none text-sm sm:text-base"
                ...
                <div>
                  <label htmlFor="message" className="block text-sm font-medium text-zinc-600 dark:text-zinc-400 mb-2">
                    Message
                  </label>
                  <textarea
                    id="message"
                    ...
                    rows={4}
                    className="input-field w-full px-3 sm:px-4 py-2.5 sm:py-3 rounded-lg focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/50 transition-all outline-none resize-none text-sm sm:text-base"
